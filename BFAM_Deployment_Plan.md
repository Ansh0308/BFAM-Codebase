...
> To publish only an app UI change afterwards (no backend, database or VM), run `powershell -ExecutionPolicy Bypass -File .\deploy\publish-web.ps1`; it takes about two minutes.
...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Mobile-web (start here)** | $0                                                                                                                 | Open the link in **Safari → Share → Add to Home Screen** | Full booking / match / live-score flow. No native payments, camera QR check-in, push or contacts. The site ships a web-app manifest, a BFAM icon and the Apple home-screen tags, so it installs as a full-screen app with an icon (verified in the built HTML; not yet tried on a physical iPhone). iOS web apps can also receive push notifications from iOS 16.4, which is a free alternative to native push. |
