...
export default function ManageTurfScreen() {
  ...
  // Operating Hours — required or bookingService rejects every slot on
  ...
  async function saveOperatingHours() {
    ...
    try {
      const rows: SetOperatingHoursRow[] = Object.entries(hoursByDay)
        ...
        .map(([day, v]) => ({ day_of_week: Number(day), open_time: v.open, close_time: v.close }));
  ...
  return (
    <ScreenContainer scroll>
      ...
      {turf.venue_id ? (
        <>
          <Text className="font-ui text-micro text-text-tertiary mb-3" testID="turf-venue-note">
          ...
          />
          <TextField label="City" value={city} editable={false} testID="edit-turf-city" />
        </>
      ) : (
        <>
          <TextField
      ...
      {!turf.venue_id && venues.length > 0 && (
        <View className="mt-6">
          ...
          <ChipSelect
            ...
            value={assignVenueId}
            onChange={setAssignVenueId}
            testID="assign-venue-select"
          />
          <Button
            label="Assign to Venue"
            variant="secondary"
            onPress={assignToVenue}
            loading={assigning}
        ...
        <View key={day} className="flex-row items-center mb-2">
          ...
          <View className="flex-1 mr-2">
            <TextField
              ...
              onChangeText={(v) => setDayHours(day, 'open', v)}
              placeholder="06:00"
              testID={`hours-open-${day}`}
            />
          </View>
          <View className="flex-1">
            <TextField
              ...
              onChangeText={(v) => setDayHours(day, 'close', v)}
              placeholder="23:00"
              testID={`hours-close-${day}`}
            />
          </View>
        </View>
      ))}
